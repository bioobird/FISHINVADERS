...
def update_display(game_state):
    if game_state == "start":
        ...
        # Draw the yellow circle for boss stages
        if current_stage in [3, 4, 5]:
            YELLOW_CIRCLE_X = WINDOW_WIDTH - 150  # Adjust the X position as needed
            YELLOW_CIRCLE_Y = 50  # Adjust the Y position as needed
            YELLOW_CIRCLE_RADIUS = 100  # Adjust the radius as needed
            pygame.draw.circle(window, YELLOW, (YELLOW_CIRCLE_X, YELLOW_CIRCLE_Y), YELLOW_CIRCLE_RADIUS)
